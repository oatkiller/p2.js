...
Capsule.prototype.raycast = function(result, ray, position, angle) {
  ...
  for (var i = 0; i < 2; i++) {
    ...
    if (delta < 0) {
      ...
      if (
        vec2.squaredDistance(hitPointWorld, position) > diagonalLengthSquared
      ) {
...
Capsule.prototype.pointTest = function(localPoint) {
  ...
  if (
    Math.abs(localPoint[0]) <= halfLength &&
    Math.abs(localPoint[1]) <= radius
  ) {
    return true;
  }

  if (
    Math.pow(localPoint[0] - halfLength, 2) + Math.pow(localPoint[1], 2) <=
    radius * radius
  ) {
    return true;
  }
